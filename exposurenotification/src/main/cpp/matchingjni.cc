...
JND(matchingNativeGAENP)(JNIEnv *env, jclass clazz, jlong native_ptr,
                         jobjectArray teks, jobjectArray locs, jintArray intervals, jboolean gaenplus) {
  exposure::MatchingHelper *wrapper =
          reinterpret_cast<exposure::MatchingHelper *>(native_ptr);

  LOG_I("tekVector");
  std::vector<uint8_t*> tekVector;
  for (int i=0; i<env->GetArrayLength(teks); ++i) {
    jbyteArray jba = (jbyteArray) env->GetObjectArrayElement(teks, i);
    jbyte* c_array = env->GetByteArrayElements(jba, 0);
    tekVector.push_back((uint8_t *) c_array);
  }

  LOG_I("LocVector");
  std::vector<uint8_t*> locVector;
  for (int i=0; i<env->GetArrayLength(locs); ++i) {
    jbyteArray jba = (jbyteArray) env->GetObjectArrayElement(locs, i);
    jbyte* c_array = env->GetByteArrayElements(jba, 0);
    locVector.push_back((uint8_t *) c_array);
  }

  LOG_I("IntervalVector");
  std::vector<int> intervalVector;
  jint* intArray = env->GetIntArrayElements(intervals, nullptr);
  for (int i=0; i<env->GetArrayLength(intervals); ++i)
    intervalVector.push_back(intArray[i]);

  bool gp = (bool)(gaenplus == JNI_TRUE);

  return wrapper->MatchingGAENP(env, tekVector, locVector, intervalVector, gp);
}
