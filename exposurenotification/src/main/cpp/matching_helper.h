...
namespace exposure {
    ...
    public:
        ...
        bool GenerateIds_GAENP(const uint8_t *diagnosis_key, uint32_t rolling_start_number,
                               uint8_t* loc, uint8_t *ids);

        inline jint LastProcessedKeyCount() const { return last_processed_key_count; }

        // Added for GAEN+
        jobjectArray MatchingGAENP(JNIEnv *env, const std::vector<uint8_t*> &teks, const std::vector<uint8_t*> &locs, const std::vector<int> &intervals, bool gp);
