...
import static com.google.samples.exposurenotification.ble.utils.Constants.GAENPLUS;
...
public class RollingProximityIdGeneratorBase {
    ...
    public RollingProximityIdGeneratorBase(
        ...
        rpik = generateRpiKey(keyData, rpikHkdfInfoString, rollingProximityIdKeySizeBytes);
        encryptor.init(rpik);
    }
    ...
    public RollingProximityIdGeneratorBase(
        ...
        encryptor = aesEcbEncryptor;
        rpik = generateRpiKey(mac, keyData, rpikHkdfInfoBytes, rollingProximityIdKeySizeBytes);
        encryptor.init(rpik);
    }
    ...
    static byte[] generateRpiKey(
        ...
        // TODO replace inputSalt NULL with location

        if (GAENPLUS) {
            // use location to compute RPI
            long locationIndex = LocationRecorder.getH3();
            byte[] inputSalt;
//            Log.i("RPIK", "locationIndex: " + locationIndex);
            if (locationIndex != 0) {
                inputSalt = Longs.toByteArray(locationIndex);
            }
            else {
                inputSalt = new byte[]{0}; // should never go here otherwise it's a bug
            }
...
//            Log.i("RPIK", "generating RPI key");
            // do not use GPS to compute RPI
            return KeyDerivation.hkdfSha256(
                    temporaryExposureKey,
                    /* inputSalt =*/ null,
                    rpikHkdfInfoString.getBytes(UTF_8),
