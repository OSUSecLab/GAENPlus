...
public class BleAdvertisementGenerator implements BleKeyGenerator {
    private RollingProximityIdManager proxIdManager;

    public BleAdvertisementGenerator(RollingProximityIdManager proxIdManager) {
        this.proxIdManager = proxIdManager;
    }

    public RollingProximityId getRPI(int interval) {
        try {
            return proxIdManager.getCurrentRollingProximityId(interval);
        } catch (CryptoException e) {
            e.printStackTrace(); // should not go here
        }
        return null;
    }

    @Override
    public AdvertisementPacket generatePacket(int interval) throws CryptoException {
        // byte 0 - version
    ...
    @VisibleForTesting
    AdvertisementPacket generatePacket(BluetoothMetadata metadata, int interval) throws CryptoException {
        RollingProximityId rollingProximityId = proxIdManager.getCurrentRollingProximityId(interval);
        ...
        AssociatedEncryptedMetadataGenerator aemGenerator = new AssociatedEncryptedMetadataGenerator(proxIdManager.getTemporaryExposureKey());
